...
export async function getPublisherCategoriesController(
  request: Request,
  response: Response,
): Promise<void> {
  try {
    ...
    const keyword = request.query.keyword
      ...
      : undefined;
    const trangThai = request.query.trangThai
      ? String(request.query.trangThai)
      : undefined;

    const result = await getPublisherCategory(tableName, {
      page,
      limit,
      keyword,
      trangThai,
    ...
    response.status(200).json({
      status: "success",
      message: "Lay danh sach du lieu Publisher thanh cong",
      data: result,
    });
  } catch (error) {
    handleControllerError(response, error);
